...
- 中英文是两套地址：中文 `/`、`/<app-id>.html`，英文 `/en/`、`/en/<app-id>.html`，互相用 hreflang 标注。首页正文在 `_includes/home.html`，两种语言共用。
- `privacy.html`、`terms.html` 是单地址页内切换语言，不走模板。
...
4. 在 `en/` 下建同名英文页，只要 4 行 front matter（复制任一 `en/<app-id>.md` 改 `app_id`），正文自动取自中文页的英文块。
5. 还在审核的 App 在 `apps.yml` 里加 `coming_soon: true`，下载按钮会换成「即将上架」；上架后删掉。
6. 跑一次字体子集（见下）。
